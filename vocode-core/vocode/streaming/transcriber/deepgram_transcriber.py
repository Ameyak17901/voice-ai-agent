...
class DeepgramTranscriber(BaseAsyncTranscriber[DeepgramTranscriberConfig]):
    ...
    def get_deepgram_url(self):
        ...
        model = self.transcriber_config.model or "nova-2"
        if model == "nova":
            model = "nova-2"
    ...
    async def _run_loop(self):
        restarts = 0
        while not self._ended and restarts < 50:
            loop_start = time.time()
            try:
                await self.process()
            except Exception as e:
                logger.error(f"[VOICE_PIPELINE: DEEPGRAM_PROCESS_ERROR] Deepgram process crashed: {e}", exc_info=True)
            if time.time() - loop_start > 10:
                restarts = 0
            else:
                restarts += 1
                await asyncio.sleep(0.5)
            logger.warning(f"Deepgram connection died, restarting, num_restarts: {restarts}")
    ...
    def _compute_is_endpoint_and_log_params(
        ...
        if isinstance(
            ...
            if isinstance(deepgram_response, DeepgramUtteranceEnd):
                ...
                if deepgram_response.speech_final and (deepgram_response.top_choice.transcript or current_buffer.strip()):
                    log_params["source"] = "speech_final"
                    return True, log_params
                elif (
                    deepgram_response.is_final
                    and deepgram_response.top_choice.transcript
                    and deepgram_response.top_choice.transcript.strip()[-1] in PUNCTUATION_TERMINATORS
                ):
            ...
            async with websockets.connect(
                ...
                async def sender(
                    ...
                    while not self._ended:
                        try:
                            data = await asyncio.wait_for(self._input_queue.get(), 4.0)
