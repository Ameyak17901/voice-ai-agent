...
try:
    import miniaudio
    MiniaudioDecodeError = miniaudio.DecodeError
except (ImportError, OSError):
    miniaudio = None
    MiniaudioDecodeError = Exception
...
class MiniaudioWorker(ThreadAsyncWorker[Union[bytes, None]]):
    ...
    def __init__(
        ...
        if miniaudio is None:
            raise RuntimeError(
                "miniaudio is not available on this platform (blocked by Windows Application Control policy or not installed)."
            )
        super().__init__()
    ...
    def _run_loop(self):
        ...
        while not self._ended:
            ...
            except MiniaudioDecodeError as e:
                # TODO: better logging
