    ...
    async def _run_loop(self):
        while True:
            ...
            if item.is_interrupted():
                ...
                logger.warning(
                    f"[VOICE_PIPELINE: OUTPUT_CHUNK_INTERRUPTED] Skipping audio playback: "
                    f"chunk of {len(audio_chunk.data)} bytes was marked as interrupted!"
                )
            ...
            try:
                ...
                from loguru import logger
                logger.warning(f"Error in on_play callback: {e}")
