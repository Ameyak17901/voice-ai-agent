...
class RespondAgent(BaseAgent[AgentConfigType]):
    ...
    async def handle_generate_response(
        ...
        logger.info(
            f"[VOICE_PIPELINE: 3. LLM_GENERATE_START] Invoking LLM generation for query: '{transcription.message}'"
        )
        ...
        async for generated_response in responses:
            ...
            if isinstance(generated_response.message, BaseMessage):
                ...
                logger.info(
                    f"[VOICE_PIPELINE: 4. LLM_RESPONSE_CHUNK] LLM response chunk: '{generated_response.message.text}'"
                )
        ...
        logger.info(
            f"[VOICE_PIPELINE: 4. LLM_RESPONSE_COMPLETE] Full LLM response generated: '{responses_buffer.strip()}'"
        )
    ...
    async def process(self, item: InterruptibleEvent[AgentInput]):
        ...
        try:
            ...
            if isinstance(agent_input, TranscriptionAgentInput):
                ...
                logger.info(
                    f"[VOICE_PIPELINE: 3. LLM_QUERY_INGRESS] Agent received query: '{transcription.message}' "
                    f"(confidence={transcription.confidence:.2f}, is_interrupt={transcription.is_interrupt})"
                )
            ...
            logger.warning(
                "[VOICE_PIPELINE: CANCELLED] BaseAgent.process was CANCELLED while generating response!"
            )
            pass
