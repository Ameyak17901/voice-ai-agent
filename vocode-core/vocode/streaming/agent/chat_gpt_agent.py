...
class ChatGPTAgent(RespondAgent[ChatGPTAgentConfigType]):
    ...
    async def generate_response(
        ...
        # Providers like Google Gemini reject requests ending with a model turn (HTTP 400)
        if openai_chat_messages and openai_chat_messages[-1].get("role") == "assistant":
            if "gemini" in str(chat_parameters.get("model", "")).lower():
                openai_chat_messages.append({"role": "user", "content": human_input or "Please assist."})
        ...
        logger.info(
            f"[VOICE_PIPELINE: 3. LLM_QUERY_RECEIVED] ChatGPTAgent received query: '{human_input}' "
            f"(length={len(human_input)} chars, is_interrupt={is_interrupt}, bot_was_in_medias_res={bot_was_in_medias_res})"
        )
        ...
        model_name = chat_parameters.get("model", "unknown")
        last_user_msg = next((m.get("content") for m in reversed(openai_chat_messages) if m.get("role") == "user"), human_input)
        sys_prompt_len = len(openai_chat_messages[0].get("content", "")) if (openai_chat_messages and openai_chat_messages[0].get("role") == "system" and isinstance(openai_chat_messages[0].get("content"), str)) else 0
        logger.info(
            f"[VOICE_PIPELINE: 3. LLM_QUERY_SENT] Dispatched payload to LLM model='{model_name}':\n"
            f"  - Active User Query: '{last_user_msg}'\n"
            f"  - History Context Turns: {len(openai_chat_messages)}\n"
            f"  - System Prompt Size: {sys_prompt_len} chars\n"
            f"  - Temperature: {chat_parameters.get('temperature', 'default')}, Max Tokens: {chat_parameters.get('max_tokens', 'default')}"
        )
        ...
        using_input_streaming_synthesizer = (
            ...
            if getattr(self, "conversation_state_manager", None) is not None
            else False
        )
        ...
        async for message in response_generator(
            ...
            if isinstance(message, str):
                logger.info(
                    f"[VOICE_PIPELINE: 4. LLM_RESPONSE_SENTENCE] Sentence received from LLM: '{message}'"
                )
                yield ResponseClass(
