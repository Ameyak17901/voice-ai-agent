...
from vocode.streaming.models.transcript import TranscriptEvent, Sender
...
class StreamingConversation(AudioPipeline[OutputDeviceType]):
    ...
    class TranscriptionsWorker(AsyncQueueWorker[Transcription]):
        ...
        async def process(self, transcription: Transcription):
            ...
            if transcription.message.strip() == "":
                logger.info("[VOICE_PIPELINE: STT_DROP_EMPTY] Discarded empty/whitespace transcription from STT; NOT sent to LLM.")
                return
            logger.info(
                f"[VOICE_PIPELINE: 2. STT_INGRESS] TranscriptionsWorker received: '{transcription.message}' "
                f"(is_final={transcription.is_final}, confidence={transcription.confidence:.2f})"
            )
            # Handle utterances during the initial message with barge-in support
            initial_message_ongoing = not self.conversation.initial_message_tracker.is_set()
            if initial_message_ongoing:
                if self.conversation.is_interrupt(transcription):
                    logger.info(f"Barge-in: human interrupted initial greeting with: '{transcription.message}'")
                    await self.conversation.broadcast_interrupt()
                    self.conversation.initial_message_tracker.set()
                    initial_message_ongoing = False
                else:
                    logger.info(
                        f"[VOICE_PIPELINE: STT_DROP_INITIAL_GREETING_OVERLAP] Utterance '{transcription.message}' occurred "
                        f"during initial greeting without meeting interrupt threshold (confidence={transcription.confidence:.2f}); NOT sent to LLM."
                    )
                    self.has_associated_ignored_utterance = not transcription.is_final
            ...
            if self.should_ignore_utterance(transcription):
                logger.info(
                    f"[VOICE_PIPELINE: STT_DROP_BACKCHANNEL] Bot is still speaking; utterance '{transcription.message}' "
                    f"classified as backchannel/low-sensitivity; NOT sent to LLM."
                )
            ...
            if self.ignore_next_message and transcription.is_final:
                ...
                logger.info(
                    f"[VOICE_PIPELINE: STT_DROP_IGNORE_NEXT] ignore_next_message is active; discarding '{transcription.message}'; NOT sent to LLM."
                )
            ...
            if transcription.is_final:
                ...
                wpm_val = f"{transcription.wpm():.1f}" if transcription.wpm() is not None else "N/A"
                logger.info(
                    f"[VOICE_PIPELINE: 2. STT_FINAL_DISPATCH] User utterance complete: '{transcription.message}' "
                    f"(confidence={transcription.confidence:.2f}, wpm={wpm_val})"
                )
            if not self.conversation.is_human_speaking:
                ...
                if self.conversation.current_transcription_is_interrupt:
                    logger.info(
                        f"[VOICE_PIPELINE: STT_INTERRUPT_TRIGGER] Transcription triggered interrupt: '{transcription.message}', confidence: {transcription.confidence}"
                    )
            ...
            if not transcription.is_final and transcription.message and transcription.message.strip():
                logger.info(
                    f"[VOICE_PIPELINE: 2. STT_INTERIM] User speaking (interim): '{transcription.message.strip()}' "
                    f"(confidence={transcription.confidence:.2f}) -> In-progress partial; NOT sent to LLM yet (awaiting silence endpointing/final)."
                )
                if self.conversation.events_manager:
                    self.conversation.events_manager.publish_event(
                        TranscriptEvent(
                            text=transcription.message.strip(),
                            sender=Sender.HUMAN,
                            timestamp=time.time(),
            ...
            if transcription.is_final:
                ...
                logger.info(
                    f"[VOICE_PIPELINE: 3. TRANSCRIPTION_ENQUEUED_TO_AGENT] Enqueueing query '{transcription.message}' "
                    f"to agent input queue (conversation_id={self.conversation.id})"
                )
    ...
    class FillerAudioWorker(InterruptibleWorker[InterruptibleAgentResponseEvent[FillerAudio]]):
        ...
        async def process(self, item: InterruptibleAgentResponseEvent[FillerAudio]):
            try:
                ...
                logger.info(f"--------------done-----------------: {item.agent_response_tracker}")
        ...
        async def process(self, item: InterruptibleAgentResponseEvent[AgentResponse]):
            ...
            try:
                ...
                if isinstance(
                    ...
                    logger.info(
                        f"[VOICE_PIPELINE: 5. TTS_SYNTHESIS_INGRESS] Requesting TTS speech synthesis for: '{agent_response_message.message.text}'"
                    )
                ...
                if maybe_synthesis_result is not None:
                    ...
                    logger.info(
                        f"[VOICE_PIPELINE: 5. TTS_SYNTHESIS_ENQUEUED] Enqueued synthesis result to SynthesisResultsWorker"
                    )
                    self.consumer.consume_nonblocking(
                        self.interruptible_event_factory.create_interruptible_agent_response_event(
                ...
                logger.warning(
                    "[VOICE_PIPELINE: CANCELLED] AgentResponsesWorker was CANCELLED while processing synthesis request!"
                )
                pass
    ...
    class SynthesisResultsWorker(
        ...
        async def process(
            ...
            try:
                ...
                logger.info(
                    f"[VOICE_PIPELINE: 6. TTS_PLAYBACK_START] Starting speech playback on output device for: '{message.text}'"
                )
                ...
                item.agent_response_tracker.set()
                logger.info(
                    f"[VOICE_PIPELINE: 6. TTS_PLAYBACK_FINISHED] Spoken: '{message_sent}', cut_off={cut_off}"
                )
                if cut_off:
                ...
                logger.warning(
                    "[VOICE_PIPELINE: CANCELLED] SynthesisResultsWorker was CANCELLED while playing speech to output device!"
                )
                pass
    ...
    async def start(self, mark_ready: Optional[Callable[[], Awaitable[None]]] = None):
        ...
        logger.info(f"initial_message:------------------------------------------{initial_message}, initial_message")
        if initial_message:
            asyncio_create_task(
    ...
    async def action_on_idle(self):
        logger.info("Conversation idle for too long, commencing graceful departure")
        goodbye_text = getattr(self.agent.get_agent_config(), "idle_goodbye_message", None)
        if goodbye_text:
            logger.info(f"Synthesizing graceful departure message on idle: '{goodbye_text}'")
            goodbye_tracker = asyncio.Event()
            await self.send_single_message(
                message=BaseMessage(text=goodbye_text),
                message_tracker=goodbye_tracker,
                is_interruptible=False,
            )
            try:
                await asyncio.wait_for(goodbye_tracker.wait(), timeout=5.0)
    ...
    async def check_for_idle(self):
        """Asks if human is still on the line with progressive re-engagement, and terminates gracefully if not."""
        await self.initial_message_tracker.wait()
        # Reset idle timer so initial greeting synthesis & playback does not penalize user
        self.mark_last_action_timestamp()
        check_human_present_count = 0
        agent_config = self.agent.get_agent_config()
        check_human_present_threshold = agent_config.num_check_human_present_times
        idle_messages = getattr(agent_config, "idle_messages", None) or CHECK_HUMAN_PRESENT_MESSAGE_CHOICES
        logger.info(
            f"Idle watchdog activated: threshold={self.idle_time_threshold}s, retry_limit={check_human_present_threshold}, tick=1.0s"
        )
        while self.is_active():
            await asyncio.sleep(1.0)
            if check_human_present_count > 0 and self.is_human_still_there:
                # Reset the counter if the human responded
                check_human_present_count = 0

            # Guard 1: Never nudge while human is actively speaking
            if getattr(self, "is_human_speaking", False):
                self.mark_last_action_timestamp()
                check_human_present_count = 0
                self.is_human_still_there = True
                continue

            # Guard 2: Never nudge while bot is actively speaking or synthesizing
            if self.is_bot_in_medias_res():
                self.mark_last_action_timestamp()
                continue
            ...
            if (
                ...
                if check_human_present_count >= check_human_present_threshold:
                    logger.info(
                        f"Idle threshold reached ({time.time() - self.last_action_timestamp:.1f}s > {self.idle_time_threshold}s, retries={check_human_present_count}). Terminating session."
                    )
                ...
                nudge_text = (
                    idle_messages[check_human_present_count]
                    if check_human_present_count < len(idle_messages)
                    else random.choice(CHECK_HUMAN_PRESENT_MESSAGE_CHOICES)
                )
                logger.info(
                    f"Idle watchdog: Triggering progressive nudge #{check_human_present_count + 1}: '{nudge_text}'"
                )
                await self.send_single_message(
                    message=BaseMessage(text=nudge_text),
                )
    ...
    def is_bot_in_medias_res(self) -> bool:
        if hasattr(self, "transcriptions_worker") and self.transcriptions_worker:
            return self.transcriptions_worker.is_bot_in_medias_res()
        return False

    def is_bot_speaking(self) -> bool:
        if hasattr(self, "transcriptions_worker") and self.transcriptions_worker:
            return self.transcriptions_worker.is_bot_still_speaking()
        return False
                ...
                try:
                    ...
                    if not interruptible_event.is_interrupted():
                        if interruptible_event.interrupt():
                            logger.info(
                                f"[VOICE_PIPELINE: INTERRUPT_EVENT] Interrupted event: {type(interruptible_event.payload).__name__}"
                            )
                            num_interrupts += 1
                except queue.Empty:
                    break
            logger.info(
                f"[VOICE_PIPELINE: BROADCAST_INTERRUPT] broadcast_interrupt triggered! "
                f"num_interrupts={num_interrupts}. Cancelling agent and agent_responses_worker."
            )
    ...
    async def send_speech_to_output(
        ...
        logger.info(f"[VOICE_PIPELINE: 6. TTS_OUTPUT_START] Sending speech '{message[:50]}...' to output device")
        ...
        async for chunk_idx, chunk_result in enumerate_async_iter(synthesis_result.chunk_generator):
            if stop_event.is_set():
                logger.warning(
                    f"[VOICE_PIPELINE: OUTPUT_INTERRUPTED] Stop event set after {total_chunks_sent} chunks "
                    f"({total_bytes_sent} bytes) sent."
                )
                interrupted_before_all_chunks_sent = True
                break
            total_chunks_sent += 1
            total_bytes_sent += len(chunk_result.chunk)
            processed_event = asyncio.Event()
        ...
        logger.info(
            f"[VOICE_PIPELINE: 6. TTS_OUTPUT_SENT] Sent {total_chunks_sent} audio chunks "
            f"({total_bytes_sent} bytes) to output device. Waiting for playback..."
        )
